...
methods {
    ...
    function _.getWithdrawDelay(uint256 totalSupply, uint256 withdrawAmount, address user) external => getWithdrawDelayCVL(totalSupply, withdrawAmount, user) expect uint256;
}
...
function getWithdrawDelayCVL(uint256 totalSupply, uint256 withdrawAmount, address user) returns (uint256) {
...
// `queueWithdrawal()` revets when expected
rule unit_queueWithdrawal_revertConditions() {
    env e;

    uint256 amount;
    uint256 maxWithdrawDelay = max_uint256;

    applySafeAssumptions(e);
    require exitWindow(e) <= SECONDS_IN_WEEK() * 10;
    ...
    bool hasUserApprovedEnoughShares = jDola.allowance(e, e.msg.sender, currentContract) >= amount;
    bool isInitialized = currentContract.vault != 0;
    bool isContractBalanceOverflow = jDola.balanceOf(e, currentContract) + amount > max_uint256;
    bool isTotalWithdrawAmountOverflow = amount + withdrawAmount > max_uint256;
    bool hasUserEnoughShares = jDola.balanceOf(e, e.msg.sender) >= amount;
    bool isTotalWithdrawAmountMulFeeOverflow = (amount + withdrawAmount) * withdrawFeeBps(e) > max_uint256;
    bool isAmountMulFeeOverflow = amount * withdrawFeeBps(e) > max_uint256;
    bool isTotalWithdrawAmountZero = amount + withdrawAmount == 0;
    ...
    bool isExpectedToRevert = 
        isEtherSent ||
        !hasUserApprovedEnoughShares ||
        !isInitialized ||
        isContractBalanceOverflow ||
        isTotalWithdrawAmountOverflow ||
        !hasUserEnoughShares ||
        isTotalWithdrawAmountMulFeeOverflow ||
        isAmountMulFeeOverflow ||
        isTotalWithdrawAmountZero;

    queueWithdrawal@withrevert(e, amount, maxWithdrawDelay);
